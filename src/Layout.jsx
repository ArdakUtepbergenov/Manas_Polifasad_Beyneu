      ...
      <header className={`fixed top-0 inset-x-0 z-40 transition-all duration-500 ${navClass}`}>
        <Container size="wide" className="flex items-center justify-between h-[72px] md:h-[84px]">
          ...
          <div className="hidden lg:flex items-center gap-3">
            <a href="tel:+77027829474" className={`flex items-center gap-2 text-[14px] font-medium tracking-tight ${wantsDark ? 'text-white' : 'text-ink-900'}`}>
              <Icons.Phone size={16} />
              <span>+7 (702) 782-94-74</span>
            </a>
    ...
    >
      ...
      <div className={`absolute top-[72px] inset-x-4 rounded-3xl bg-white shadow-soft-lg p-6 transition-all duration-500
        ...
        <div className="mt-6 grid grid-cols-2 gap-3">
          ...
          <Button href="tel:+77027829474" as="a" variant="outline" size="md" icon={<Icons.Phone size={15}/>} iconAfter={null}>Позвонить</Button>
        </div>
        <div className="mt-5 pt-5 border-t border-ink-100 grid grid-cols-2 gap-3 text-[13px] text-ink-500">
          <a href="https://wa.me/77027829474" className="flex items-center gap-2 hover:text-ink-900">
